...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    model = Sequential()
    model.add(Dense(train_conf['dense_dim'], activation='relu', input_shape=(768,))) # BERT embedding -> depend du type d'embedding
    ...
    with open(os.path.join(artefacts_path, "labels.json"), 'w') as labels_file:
        print(label_index_map)
        json.dump(label_index_map, labels_file, default=int)
